...
Before any Codex marketplace install, confirm the final long-term `user_plugin/<tool-name>` path. Do not let the marketplace download location become the durable home just because the host chose it automatically.

If a Codex marketplace install places a third-party repo under a vendor-managed cache or temp path such as `~/.codex/.tmp/marketplaces/<tool-name>`, `C:\Users\<user>\.codex\...`, or another user-profile path, treat that path as an entry location, not the long-term real home. Preferred handling:
...
5. write the inventory as a Chinese-primary bilingual quick reference:
   ...
   - mark each skill with its source project when multiple entries come from the same known upstream repo or plugin bundle
   - use `来源待确认 / Source pending` for entries whose upstream cannot be verified from local evidence; do not invent a source label for them
